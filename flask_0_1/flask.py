...
#                              代码主体部分
# - 核心模块
#     - Request               # 未独立实现(继承 Werkzeug 的 Request), 支持继承后替换
#     - Response              # 未独立实现(继承 Werkzeug 的 Request), 支持继承后替换
#     - Flask                 # 框架核心部分
#     - _RequestContext       # 请求上下文(请求开始被 push 入栈中, 请求结束后从栈中 pop 出来)
#
# - 全局对象
#     - _request_ctx_stack    # 请求本地栈, 栈顶元素则为当前请求上下文
#     - current_app
#     - request
#     - session
#     - g                     # 当前请求上下文中的全局变量
...
###############################################################################
#
# 请求上下文环境, 将请求上下文压入请求栈中
# 这样, 便可以通过全局变量在该次请求的生命周期内获取请求上下文信息
#
# 实现了 `__enter__` 和 `__exit__`,
# 可以采用 with 语句在结束该次请求的时候自动 pop 掉该次请求的上下文
# 具体使用场景见 `request_context` 和 `wsgi_app`
#
###############################################################################
...
class Flask(object):
    ...
    ###############################################################################
    #
    # Flask 的 session 是通过 cookie 实现的, cookie 用的是 werkzeug 的 SecureCookie 实例
    #
    # open_session: 从请求中取出 cookie
    # save_session: 将 cookie 存到返回中
    #
    ###############################################################################
    ...
    # 后续版本中有加入检查, 若同一个路由节点, 绑定了两个不同的视图函数, 则抛出错误
    # eg. 在下面的例子中, 路由节点都是 `users`, 这时候
    ...
    # def users(): pass
    #
    # 另外, 这里不太理解为什么作者没有把<路由节点-视图函数>绑定在 `add_url_rule` 中一起实现
    # 而是需要另外显式调用, 大概是让使用者注意不要为同一个路由节点绑定不同视图函数
    #
    ...
    ###############################################################################
    #                               定义视图
    # 在 Flask 中定义视图的时候常用的 @route('/xxx', methods=['POST']) 装饰器的定义
    #
    # 主要作用是调用前面的 add_url_rule 添加路由规则, 并绑定路由节点和视图函数
    # 并且用视图函数的名字作为路由节点
    #
    ###############################################################################
    ...
    ###############################################################################
    #
    # 一个简单的装饰器, 将错误码和错误处理函数绑定
    #
    # Flask-0.1 版本只是解析了常见的Http状态码
    # 将所有的内部出错全部认为是 500
    # 详情见 dispatch_request
    #
    ###############################################################################
    ...
    # 请求前处理和请求后处理
    # 也是一个简单的装饰器
    # 将定义的请求前函数按照定义的顺序插入列表中, 在处理请求前先运行
    # 将定义的请求后函数按照定义的顺序插入列表中, 在处理完请求后返回 response 前运行
    #
    # 详情见 `preprocess_request` 和 `process_response`
    #
    ###############################################################################
    ...
    ###############################################################################
    #
    # 路由解析: endpoint --> view_function, 由 Flask 自身完成
    # 解析完以后获取到 view_function, 调用 view_function
    #
    # 错误处理: error_handler
    # 捕获 HTTPException, 这里捕获的异常基本上是在 self.match_request() 中发生
    # Web 程序运行中抛出的错误, 当做500处理, 返回 500
    #
    # 抛出的错误会被 error_handler 处理
    #
    ###############################################################################
    ...
    ###############################################################################
    #
    # 根据传入的参数创建一个用于返回的 response 对象
    # TODO: 需要深入理解 `self.response_class.force_type(rv, request.environ)`
    #
    ###############################################################################
    ...
    ###############################################################################
    #
    # 在处理请求后, 返回 response 前按照定义的顺序执行请求后函数
    #
    ###############################################################################
